"use client";

import { ResponsiveBar } from "@nivo/bar";
import { ChartFrame, chartColor, nivoTheme } from "./chart-frame";
import { DEMO_ROWS } from "./demo-data";
import { fitBuckets } from "./domain/aggregate";

/** Live fit-distribution demo — fit ratings across the five 20-point bands. */
export function FitHistogramDemo() {
  const data = fitBuckets(DEMO_ROWS);
  const total = data.reduce((a, b) => a + b.count, 0);
  return (
    <ChartFrame
      title="Fit distribution"
      empty={total === 0}
      caption="Number of applications per fit-rating band"
      head={["Fit range", "Count"]}
      body={data.map((d) => [d.range, d.count])}
    >
      {(animate) => (
        <ResponsiveBar
          data={data.map((d) => ({ range: d.range, count: d.count }))}
          keys={["count"]}
          indexBy="range"
          margin={{ top: 8, right: 8, bottom: 32, left: 32 }}
          padding={0.3}
          colors={[chartColor(1)]}
          enableLabel={false}
          animate={animate}
          theme={nivoTheme}
          role="img"
          ariaLabel="Fit distribution"
        />
      )}
    </ChartFrame>
  );
}
